...
public interface AutowireCapableBeanFactory extends BeanFactory {
	...
	/**
	 * 表示没有外部定义的自动连线的常数。请注意，BeanFactoryAware etc和注释驱动的注入仍将应用。
	 ...
	 * @see #AUTOWIRE_NO
	 * 
	 * 按名称或类型自动连接给定bean实例的bean属性。也可以使用AUTOWIRE_NO调用，以便在实例化回调之后应用（例如，对于注释驱动的注入）。
	 * 不应用标准BeanPostProcessors回调或执行bean的任何进一步初始化。这个接口为这些目的提供了不同的、细粒度的操作，例如initializeBean。
	 * 但是，如果适用于实例的配置，则会应用InstantiationWarebeanPostprocessor回调。
	 */
